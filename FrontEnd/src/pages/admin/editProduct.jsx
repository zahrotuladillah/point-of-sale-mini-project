...
function AddProduct() {
  ...
  const schema = yup.object().shape({
    ...
    dataColor: yup.array().of(
      yup.object().shape({
        ...
        hexCode: yup
          .string()
          .matches(/^#[0-9A-Fa-f]{6}$/, "Invalid hex code")
  ...
  const getProduct = (url) =>
    axios.get(url).then((res) => {
      ...
      setValue("dataPrev", res.data.dataPrev);
      setValue("dataColor", res.data.dataColor);
      setValue("dataSize", res.data.dataSize);
    });
  ...
  const onSubmitForm = (data) => {
    ...
    axios
      .put(`http://localhost:3000/product/${param.id}`, payload)
      .then(() => {
        Swal.fire({
          title: "Good job!",
          text: "Data edited!",
        ...
        <section className="content z-0 my-[4vh] max-w-[80%] m-auto flex flex-col gap-[4vh] sm:gap-[3vh] sm:my-[3vh]">
          ...
          >
            ...
            <div>
              ...
              {image.map((image, index) => (
                <div key={image.id} className="flex gap-4 mb-2">
                  ...
                  />
                  <button
                    className="rounded-lg bg-red-400 p-2 text-white self-center hover:bg-red-600"
                    type="button"
                    ...
                    <div>
                      {errors.dataPrev[index].dataPreview && (
                        <p className="error text-red-500">
                          {errors.dataPrev[index].dataPreview.message}
                        </p>
                      )}
                      {errors.dataPrev[index].color && (
                        <p className="error text-red-500">
                          {errors.dataPrev[index].color.message}
                        </p>
                      )}
                    </div>
                  )}
              ...
              <button
                type="button"
                className="rounded-lg bg-first hover:bg-yellow-600 p-2 mt-4 text-white self-center"
                onClick={() => appendImage({ dataPreview: "", color: "" })}
              >
                Add Image
              </button>
            ...
            <div>
                  ...
                  {errors.dataColor && errors.dataColor[index] && (
                    <div>
                      {errors.dataColor[index].colorName && (
                        <p className="error text-red-500">
                          {errors.dataColor[index].colorName.message}
                        </p>
                      )}
                      {errors.dataColor[index].hexCode && (
                        <p className="error text-red-500">
                          {errors.dataColor[index].hexCode.message}
                        </p>
                      )}
                    </div>
                  )}
              ...
              <button
                type="button"
                className="rounded-lg bg-first p-2 mt-4 hover:bg-yellow-600 text-white self-center"
                onClick={() => appendColor({ colorName: "", hexCode: "" })}
              >
                Add Color
              </button>
            </div>
            ...
            <div>
              ...
              {errors.dataSize && (
                <p className="error text-red-500">{errors.dataSize.message}</p>
              )}
            </div>
